...
void switchActivate(struct Switch* switchObj) {
    ...
    soundPlayerPlay(soundsButton, 3.0f, 0.9f, &switchObj->rigidBody.transform.position, &gZeroVec, SoundTypeAll);
...
void switchUpdate(struct Switch* switchObj) {
    ...
    if (switchObj->timeLeft <= 0.0f) {
        ...
        soundPlayerPlay(soundsButtonRelease, 3.0f, 0.8f, &switchObj->rigidBody.transform.position, &gZeroVec, SoundTypeAll);
