...
char usb_initialize(void)
{
    ...
    zeroMemory(usb_buffer, BUFFER_SIZE);
...
void usb_read(void* buffer, int nbytes)
{
    ...
    while (left > 0)
    {
        ...
        memCopy((void*)(((u32)buffer)+read), usb_buffer+copystart, block);
...
static void usb_everdrive_write(int datatype, const void* data, int size)
{
    ...
    while (left > 0)
    {
        ...
        memCopy(usb_buffer+offset, (void*)((char*)data+read), block);
